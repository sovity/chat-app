...
class EdcService(
    ...
    @ConfigProperty(name = "chat-app.edc.chat-app.edc.connector-endpoint")
    ...
    fun queryActiveContracts(): List<Connection> =
        edcClient.uiApi().getContractAgreementPage(ContractAgreementPageQuery(ContractTerminationStatus.ONGOING))
            .contractAgreements
            .filter { it.asset.assetId == "chat-app" && it.direction == ContractAgreementDirection.CONSUMING }
            .map {
                Connection(
                    participantId = it.counterPartyId,
                    connectorEndpoint = it.counterPartyAddress,
                    contractNegotiationId = it.contractNegotiationId,
                    contractAgreementId = it.contractAgreementId,
                    transferProcessId = it.transferProcesses.firstOrNull()
                        ?.takeIf { p -> p.state.name == "STARTED" }
                        ?.transferProcessId
                )
            }
